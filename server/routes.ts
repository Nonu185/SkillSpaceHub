...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get(
    "/api/users/:userId/skill-listings",
  ...
  app.get(
    "/api/skill-messages/listing/:listingId",
    async (req: Request, res: Response) => {
      try {
  ...
  app.get(
    "/api/skill-messages/users/:user1Id/:user2Id",
    async (req: Request, res: Response) => {
      try {
          ...
          if (parsedMessage.success) {
            ...
            clients.forEach((client) => {
              if (
                ...
                client.userId === newMessage.receiverId
              ) {
                if (client.ws.readyState === WebSocket.OPEN) {
                  client.ws.send(
                    JSON.stringify({
                      ...
                      message: {
                        ...newMessage,
                        createdAtFormatted: formatRelativeTime(
